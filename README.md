...
## Open questions
- Est-ce que si je crée le site d'abord en français et que je n'utilise pas le multi-langue j'ai moyen d'éviter d'avoir le logo odoo en bas de page ?
- x
...
## Server setup and Odoo installation

Update droplet and install Posrgres
```
sudo apt-get update
...
sudo apt-get install python3-phonenumbers
```
Create needed folders & permissions
```
...
Print log file in real-time : ```sudo tail -f /var/log/odoo/odoo-server.log```
...
See Odoo logs in real-time : ```sudo tail -f /var/log/odoo/odoo-server.log```
